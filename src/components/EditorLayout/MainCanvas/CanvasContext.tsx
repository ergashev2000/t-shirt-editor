...
export function CanvasProvider({ children }: { children: React.ReactNode }) {
  ...
  const [designArea, setDesignArea] = useState<DesignAreaConfig>(
    PRODUCT_DESIGN_AREAS[0],
  );
  const [selectedColor, setSelectedColor] = useState<ProductColor>(
    PRODUCT_DESIGN_AREAS[0].availableColors[0],
  );
  const [isRemovingBg, setIsRemovingBg] = useState(false);
  const [removingBgElementId, setRemovingBgElementId] = useState<string | null>(
    null,
  );
  ...
  const saveToHistory = useCallback(
    (newElements: CanvasElement[], newSelected: string | null) => {
  ...
  const isPartiallyVisible = useCallback(
  ...
  const snapBackIfOutside = useCallback(
  ...
  const addElement = useCallback(
    async (
      ...
      if (position) {
        const snapped = snapBackIfOutside(
          position.x,
          position.y,
          width,
          height,
  ...
  const toggleLock = useCallback(() => {
    ...
    setElements((prev) => {
      const newElements = prev.map((el) =>
        el.id === selectedEl.id ? { ...el, locked: !el.locked } : el,
      );
      saveToHistory(newElements, selectedElement);
      return newElements;
    });
  ...
  const alignRight = useCallback(() => {
    ...
    updateElementWithHistory(selectedEl.id, {
      x: designArea.x + designArea.width - selectedEl.width,
    });
  }, [selectedEl, updateElementWithHistory, designArea]);
  ...
  const alignBottom = useCallback(() => {
    if (!selectedEl) return;
    updateElementWithHistory(selectedEl.id, {
      y: designArea.y + designArea.height - selectedEl.height,
    });
  }, [selectedEl, updateElementWithHistory, designArea]);
  ...
  const startCropping = useCallback(() => {
    if (
      ...
      removingBgElementId === selectedEl.id
    )
      return;
    setCropBox({
      x: 0,
